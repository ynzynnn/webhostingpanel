    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="card-box p-5 space-y-4">
                ...
                @else
                    <div class="p-3.5 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-xs space-y-1">
                        ...
                        <p class="text-[11px] text-amber-700">Pastikan A Record DNS domain {{ $website->domain_name }} sudah mengarah ke IP server ini sebelum menerbitkan SSL.</p>
                    </div>

                    <form method="POST" action="{{ route('websites.issue-ssl', $website) }}">
                        @csrf
                        <button type="submit" class="w-full py-2.5 px-3 rounded-lg bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs shadow-xs transition-all flex items-center justify-center space-x-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                            <span>Terbitkan Sertifikat SSL</span>
                        </button>
                    </form>
                @endif
            </div>
            ...
            <div class="card-box p-5 space-y-3">
                ...
                <a href="{{ route('client.files') }}" class="w-full py-2.5 px-3 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs border border-slate-200 transition-all flex items-center justify-center space-x-2">
