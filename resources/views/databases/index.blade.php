...
@section('title', 'Manajemen Database MariaDB')
@section('page_title', 'Manajemen Database MariaDB')

@section('content')
<div class="space-y-6" x-data="{ showModal: false, dbSuffix: '', dbPass: '', generatePassword() { this.dbPass = Math.random().toString(36).slice(-10) + 'A1!' } }">

    <!-- Header Actions -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-base font-bold text-slate-900 tracking-tight">Daftar Database MariaDB</h2>
            <p class="text-xs text-slate-500 mt-0.5">Kelola database MariaDB terisolasi dengan user & password khusus per website.</p>
        </div>
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="py-3.5 px-4 text-right">
                                <form method="POST" action="{{ route('databases.destroy', $db) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus database {{ $db->name }}?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-2.5 py-1 rounded bg-rose-50 hover:bg-rose-100 text-rose-700 font-mono text-[11px] border border-rose-200 transition-all font-semibold">
                                        Hapus Database
                                    </button>
                                </form>
                            </td>
